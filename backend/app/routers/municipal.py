...
PRIORITY_SQL = text(
    ...
    SELECT
        ...
        CASE WHEN EXISTS (
            SELECT 1
            FROM crosswalks cw
            JOIN municipal_zones mz
              ON mz.zone_type IN ('school', 'hospital')
             AND ST_DWithin(mz.geom, cw.location, 80)
            WHERE cw.id = it.crosswalk_id
        ) THEN 2.0 ELSE 1.0 END AS zone_multiplier
    FROM incident_telemetry it
    WHERE (:city = '' OR it.city ILIKE :city)
      AND it.event_type IN ('hard_brake', 'occlusion_interrupt')
    GROUP BY it.crosswalk_id, it.city
    """
)


def _load_tickets(db: Session, city: str) -> list[dict]:
    tickets: list[dict] = []
    for row in db.execute(PRIORITY_SQL, {"city": city}).mappings().all():
        item = ticket_from_counts(
            row["crosswalk_id"],
            row["city"],
            row["hard_brakes"],
            row["occlusions"],
...
def prioritized_tickets(
    city: str = Query("", max_length=80), db: Session = Depends(get_db)
) -> dict:
...
def export_prioritized_tickets_csv(
    city: str = Query("", max_length=80), db: Session = Depends(get_db)
):
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": (
                f"attachment; filename=cross_safe_priority_tickets_{slug}.csv"
            )
        },
    )
