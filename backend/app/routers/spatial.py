...
def nearby_crosswalks(
    ...
    radius_m: float = Query(250, gt=0),
    speed_mps: float = Query(0, ge=0),
    ...
    for row, plat, plon in rows:
        out.append(
            CrosswalkOut(
                ...
                lon=float(plon),
                risk_score=row.risk_score,
                distance_m=round(haversine_m(lat, lon, float(plat), float(plon)), 1),
                notes=row.notes,
            )
        )
...
def ingest_telemetry(payload: TelemetryIn, db: Session = Depends(get_db)) -> TelemetryOut:
    if payload.crosswalk_id:
        exists = db.get(Crosswalk, payload.crosswalk_id)
        if not exists:
...
def preview_window(payload: NearbyQuery) -> dict:
    ...
    return {
        ...
        "search_radius_m": min(max(payload.radius_m, window + 40), settings.max_search_radius_m),
    }
